...
import { CheckCircle2, ChevronRight, Calendar, User, Phone, AlertCircle, MapPin, Car, Info, Shield, Heart, Copy, Users } from 'lucide-react';
...
export function ApplyVolunteer() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let payload = { ...formData, q8: finalQ8, category: activeTab === 'gathering' ? '모임 활동' : '유기견 봉사' };

      if (activeTab === 'gathering') {
        payload.q6 = userVolCount?.toString() || '0'; // 6. 봉사 신청 횟수 (자동 기입)
        payload.q7 = formData.q16; // 7. 카카오톡 ID
        payload.q8 = formData.q17; // 8. 개인정보 동의
        payload.q9 = formData.q15; // 9. SNS 동의
        payload.q10 = formData.q12; // 10. 환불 규정 동의
        payload.q11 = formData.q13; // 11. 입금 여부
        payload.q12 = formData.q14; // 12. 문의사항
      }
        ...
        <div style={{ background: '#FAF8F5', padding: '50px 40px', borderRadius: '4px', boxShadow: '0 20px 40px rgba(0,0,0,0.1)', maxWidth: '600px', width: '100%', border: '1px solid #E5D5B5', position: 'relative' }}>
          ...
          <div style={{ textAlign: 'center', fontSize: '16px', color: '#374151', lineHeight: '1.8', marginBottom: '40px' }}>
            ...
            <p style={{ margin: 0, fontWeight: '700', color: '#8B5A19', fontFamily: '"Pretendard", sans-serif' }}>
              {activeTab === 'gathering' ? '모임일 약 1주일 전에 팀 채팅방으로 초대드려 세부 안내를 드리겠소.' : '봉사일 약 1주일 전에 팀 채팅방으로 초대드려 집결 관련 안내를 드리겠소.'}
            </p>
          </div>
          ...
          <div style={{ borderTop: '1px solid #C8963E', borderBottom: '1px solid #C8963E', padding: '30px 0', marginBottom: '40px', textAlign: 'center' }}>
            <h3 style={{ fontSize: '20px', fontWeight: '900', color: '#1E3A5F', marginBottom: '16px', letterSpacing: '1px' }}>
              {activeTab === 'gathering' ? '참가비 입금 안내' : '봉사비(운영비) 입금 안내'}
            </h3>
                ...
                {activeTab === 'gathering' ? (
                  <>
                    <div style={{ marginBottom: '4px' }}>
                      {formData.q1 ? (() => {
                        const opt = gatherings.find(o => o.name === formData.q1);
                        return opt && opt.fee ? (isNaN(Number(opt.fee)) ? opt.fee : `${Number(opt.fee).toLocaleString()}원`) : '해당 참가비 금액';
                      })() : '해당 참가비 금액'}
                      <span style={{ fontSize: '13px', fontWeight: '400' }}> (본인 이름으로 입금)</span>
                    </div>
                    <div style={{ color: '#991B1B', fontWeight: '400', fontSize: '14px' }}>모임 집결 14일 전까지만 환불 가능</div>
                  </>
                ) : (
                  <>
    ...
    <div style={{ minHeight: '100vh', background: '#F9FAFB', paddingTop: '100px', paddingBottom: '100px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 20px' }}>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <div className="flex gap-2 md:gap-3 flex-wrap">
            <button
              ...
              className={`px-4 py-2.5 md:px-6 md:py-3 rounded-full font-bold cursor-pointer flex items-center gap-2 transition-all text-sm md:text-base ${activeTab === 'dog' ? 'bg-[#1E3A5F] text-white border-none' : 'bg-white text-gray-500 border border-gray-200 hover:bg-gray-50'}`}
            >
              {availableDates.length > 0 && <span className="bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full tracking-wide leading-none">NEW</span>}
              봉사 활동
            </button>
            <button
              onClick={() => handleTabClick('plogging')}
              className="px-4 py-2.5 md:px-6 md:py-3 rounded-full font-semibold cursor-pointer flex items-center gap-2 bg-white text-gray-400 border border-gray-200 text-sm md:text-base"
            >
              플로깅 <span className="text-[11px] font-normal">(준비중)</span>
            </button>
            <button
              onClick={() => handleTabClick('mural')}
              className="px-4 py-2.5 md:px-6 md:py-3 rounded-full font-semibold cursor-pointer flex items-center gap-2 bg-white text-gray-400 border border-gray-200 text-sm md:text-base"
            >
              벽화 봉사 <span className="text-[11px] font-normal">(준비중)</span>
            </button>
            <button
              onClick={() => handleTabClick('gathering')}
              className={`md:ml-auto px-4 py-2.5 md:px-6 md:py-3 rounded-full font-bold cursor-pointer flex items-center gap-2 transition-all text-sm md:text-base ${activeTab === 'gathering' ? 'bg-[#1E3A5F] text-white border-none' : 'bg-white text-gray-500 border border-gray-200 hover:bg-gray-50'}`}
            >
              {gatherings.length > 0 && <span className="bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-full tracking-wide leading-none">NEW</span>}
              모임 활동
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 items-stretch">
          ...
          {/* 왼쪽: 안내 문구 (공식 출정서 스타일) */}
          <div
            className="bg-[#FAF8F5] rounded shadow-[inset_0_0_60px_rgba(200,150,62,0.06),0_10px_30px_rgba(0,0,0,0.05)] border border-[#E5D5B5] relative overflow-hidden p-4 md:p-6"
          >
            {/* 모서리 금박 장식 */}
            <div className="absolute top-3 left-3 w-4 md:w-6 h-4 md:h-6 border-t-2 border-l-2 border-[#C8963E] opacity-70" />
            <div className="absolute top-3 right-3 w-4 md:w-6 h-4 md:h-6 border-t-2 border-r-2 border-[#C8963E] opacity-70" />
            <div className="absolute bottom-3 left-3 w-4 md:w-6 h-4 md:h-6 border-b-2 border-l-2 border-[#C8963E] opacity-70" />
            <div className="absolute bottom-3 right-3 w-4 md:w-6 h-4 md:h-6 border-b-2 border-r-2 border-[#C8963E] opacity-70" />

            {/* 안쪽 테두리 */}
            <div className="border border-[#C8963E]/25 p-6 md:p-10 h-full relative z-10">
              ...
              {activeTab === 'gathering' ? (
                <>
                  ...
                  <p style={{ fontFamily: '"Noto Serif KR", serif', fontSize: '18px', fontWeight: '700', color: '#2B3B4C', lineHeight: '1.6', textAlign: 'center', marginBottom: '40px', wordBreak: 'keep-all' }}>
                    "강호의 동지들, 봉사 외 모임 활동에 동참할 정예 고수를 모집하오!"
                  </p>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    <div style={{ background: 'rgba(200, 150, 62, 0.08)', padding: '24px', borderLeft: '3px solid #C8963E' }}>
                      ...
                      <ul style={{ color: '#5C4A3D', fontSize: '14px', lineHeight: '1.7', margin: 0, paddingLeft: '20px', listStyleType: 'disc' }}>
                        <li>우측 신청서 폼에서 <strong style={{ color: '#1E3A5F' }}>원하는 모임명을 클릭</strong>하시면 각 모임별 장소·일정·참가비 등 세부 안내사항을 확인할 수 있소.</li>
                        <li>참가비는 모임별로 상이하며, 환불 규정은 <strong style={{ color: '#B91C1C' }}>모임 집결 14일 전까지만 환불 가능</strong>하오.</li>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                    ...
                    <div style={{ background: 'rgba(200, 150, 62, 0.08)', padding: '24px', borderLeft: '3px solid #C8963E' }}>
                      ...
                      <ul style={{ color: '#5C4A3D', fontSize: '14px', lineHeight: '1.7', margin: 0, paddingLeft: '20px', listStyleType: 'disc' }}>
                        ...
                        <li style={{ color: '#B91C1C', fontWeight: '700', marginBottom: '8px' }}>노쇼 방지로 봉사 취소 시 환불이 불가합니다.</li>
                      </ul>
                      <div style={{ background: '#FFF8ED', border: '1px dashed #C8963E', borderRadius: '8px', padding: '12px', marginTop: '4px' }}>
                        <div style={{ color: '#8B5A19', fontSize: '13px', fontWeight: '800', marginBottom: '4px' }}>[안내] 운영진 참가비 환급</div>
                        <div style={{ color: '#5C4A3D', fontSize: '12.5px', lineHeight: '1.5', wordBreak: 'keep-all' }}>문파의 발전을 위해 헌신하는 핵심 운영진(선봉대 제외)을 위한 예우! 봉사에 출전하실 때마다 그 노고에 보답하고자 참가비 전액을 환급해 드립니다.</div>
                      </div>
                    </div>
          ...
          {/* 오른쪽: 입력 폼 */}
          <div className="bg-white p-5 md:p-10 rounded-[24px] shadow-[0_20px_40px_rgba(0,0,0,0.08)] border border-black/5">
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              ...
              <div style={{ marginBottom: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', fontSize: '14px', fontWeight: '800', color: '#1E3A5F' }}>
                  <span>신청서 작성</span>
                  <span>{step} / {activeTab === 'gathering' ? 4 : 5}</span>
                </div>
                <div style={{ width: '100%', height: '8px', background: '#F1F5F9', borderRadius: '4px', overflow: 'hidden' }}>
                  <div style={{ width: `${(step / (activeTab === 'gathering' ? 4 : 5)) * 100}%`, height: '100%', background: '#C8963E', transition: 'width 0.4s ease-in-out' }} />
                </div>
              </div>
                            ...
                            return (
                              <div key={i}>
                                ...
                                >
                                  ...
                                  />
                                  <div style={{ fontSize: '14px', color: isClosed ? '#9CA3AF' : '#374151', lineHeight: '1.5', flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    <div style={{ wordBreak: 'keep-all' }}>
                                      {isClosed ? (
                                        <span style={{ display: 'inline-block', background: '#FEE2E2', color: '#991B1B', padding: '2px 6px', borderRadius: '4px', fontSize: '12px', fontWeight: '800', marginRight: '8px', transform: 'translateY(-1px)' }}>[모집 완료]</span>
                                      ) : (
                                        <span style={{ display: 'inline-block', color: '#065F46', fontWeight: '800', background: '#D1FAE5', padding: '2px 6px', borderRadius: '4px', fontSize: '12px', marginRight: '8px', transform: 'translateY(-1px)' }}>[모집 중]</span>
                                      )}
                                      <span style={{ fontWeight: formData.q1 === opt.name ? '700' : '500', textDecoration: isClosed ? 'line-through' : 'none' }}>
                                        {opt.name}
                                      </span>
                                    </div>
                                      <div style={{ fontSize: '13px', color: '#6B7280', display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                                        <span style={{ display: 'inline-flex', alignItems: 'center', background: '#F1F5F9', padding: '4px 10px', borderRadius: '6px', fontSize: '12.5px', fontWeight: '700', color: '#475569', border: '1px solid #E2E8F0', whiteSpace: 'nowrap' }}>
                                          <Users size={14} style={{ marginRight: '6px', color: '#64748B' }} />
                                          총 {opt.capacity}명 모집
                                        </span>
                                        {opt.minCount > 0 && (
                                          <span style={{ display: 'inline-flex', alignItems: 'center', background: '#FEF3C7', padding: '4px 10px', borderRadius: '6px', fontSize: '12.5px', fontWeight: '700', color: '#92400E', border: '1px solid #FDE68A', whiteSpace: 'nowrap' }}>
                                            봉사 신청 {opt.minCount}회 이상
                                          </span>
                                        )}
                                      </div>
                                  </div>
                                ...
                                {formData.q1 === opt.name && (
                                  <div style={{ marginTop: '12px', background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '16px', fontSize: '13px', color: '#4B5563', lineHeight: '1.6' }}>
                                    <div style={{ marginBottom: '10px', display: 'flex' }}>
                                      <strong style={{ width: '65px', flexShrink: 0, color: '#1E3A5F' }}>일정</strong>
                                      <div style={{ whiteSpace: 'pre-wrap' }}>{opt.scheduleDesc || '-'}</div>
                                    </div>
                                    <div style={{ marginBottom: '10px', display: 'flex' }}>
                                      <strong style={{ width: '65px', flexShrink: 0, color: '#1E3A5F' }}>장소</strong>
                                      <div style={{ whiteSpace: 'pre-wrap' }}>{opt.location || '-'}</div>
                                    </div>
                                    <div style={{ marginBottom: '10px', display: 'flex' }}>
                                      <strong style={{ width: '65px', flexShrink: 0, color: '#1E3A5F' }}>참가비</strong>
                                      <div>{opt.fee ? (isNaN(Number(opt.fee)) ? opt.fee : `${Number(opt.fee).toLocaleString()}원`) : '무료'}</div>
                                    </div>
                                    <div style={{ display: 'flex' }}>
                                      <strong style={{ width: '65px', flexShrink: 0, color: '#1E3A5F' }}>안내사항</strong>
                                      <div style={{ whiteSpace: 'pre-wrap' }}>{opt.notice || '-'}</div>
                                    </div>
                                  </div>
                                )}
                              </div>
                            ...
                            return (
                              ...
                              >
                                <input
                                  ...
                                  style={{ width: '18px', height: '18px', accentColor: '#C8963E', flexShrink: 0 }}
                                />
                                <div style={{ fontSize: '14px', color: isClosed ? '#9CA3AF' : '#374151', lineHeight: '1.5', flex: 1 }}>
                                  <div style={{ wordBreak: 'keep-all' }}>
                                    {isClosed ? (
                                      <span style={{ display: 'inline-block', background: '#FEE2E2', color: '#991B1B', padding: '2px 6px', borderRadius: '4px', fontSize: '12px', fontWeight: '800', marginRight: '8px', transform: 'translateY(-1px)' }}>[모집 완료]</span>
                                    ) : (
                                    ...
                                    <span style={{ fontWeight: formData.date === opt.name ? '700' : '500', textDecoration: isClosed ? 'line-through' : 'none' }}>
                                      {opt.name}
                                    </span>
                                  </div>
                                </div>
                  ...
                  {activeTab === 'gathering' ? (
                    <>
                      ...
                      <div style={{ marginBottom: '24px' }}>
                        <label style={{ display: 'block', color: '#1E3A5F', fontSize: '15px', fontWeight: '800', marginBottom: '12px' }}>
                          참가비 환불 규정에 대해 확인하고 동의하시나요? <span style={{ color: '#DC2626' }}>*</span>
                        </label>
                        <div style={{ background: '#FEF2F2', padding: '16px', borderRadius: '12px', border: '1px solid #FCA5A5', fontSize: '13px', color: '#991B1B', lineHeight: '1.6', marginBottom: '12px' }}>
                          참가비는 모임 운영 및 노쇼 방지를 위해 사용되며,<br />
                          <strong style={{ fontWeight: '800' }}>참가비 환불은 모임 집결 14일 전까지만 가능</strong>함을 안내해 드립니다.
                        </div>
                        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '15px', fontWeight: '700', color: '#1E3A5F' }}>
                      ...
                      <div style={{ borderTop: '1px solid #E5E7EB', paddingTop: '24px', paddingBottom: '16px' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                          <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '15px', fontWeight: '700', color: '#1E3A5F' }}>
                            <input type="radio" name="q13" value="예" checked={formData.q13 === true || formData.q13 === '예'} onChange={(e) => setFormData(prev => ({ ...prev, q13: true }))} style={{ width: '18px', height: '18px' }} />
                            예, 본인 이름으로 입금 완료했습니다.
                          </label>
                        </div>
                      </div>
                      ...
                      <div style={{ borderTop: '1px solid #E5E7EB', paddingTop: '24px', paddingBottom: '24px' }}>
                        ...
                        <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '15px', fontWeight: '700', color: '#1E3A5F', marginBottom: '16px' }}>
              ...
              {step === 5 && (
                <>
                  ...
                  <div style={{ textAlign: 'center', padding: '24px 0 16px', borderTop: '1px solid #E5E7EB', marginTop: activeTab === 'gathering' ? '0' : '24px' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', marginTop: '12px' }}>
                ...
                >
                  {loading ? '전송 중...' : ((activeTab === 'gathering' && step === 4) || step === 5 ? '제출하기' : '다음 단계로')}
                  {!loading && !((activeTab === 'gathering' && step === 4) || step === 5) && <ChevronRight size={18} />}
                </button>
              </div>
            </form>
          </div>

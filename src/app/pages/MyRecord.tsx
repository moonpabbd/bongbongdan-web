...
export function MyRecord() { // MyRecord 탭 렌더링 로직 최종 반영 완료
  ...
  const [cancelConfirmState, setCancelConfirmState] = useState<{ scheduleText: string, category: string, msg: string } | null>(null);
  ...
  const handleChangeClick = async (scheduleText: string, dday: number | '?') => {
    if (dday !== '?' && dday < 7) {
      await showAlert('일정 변경은 봉사일 기준 최소 1주일(7일) 전까지만 가능합니다.');
      return;
    }
    ...
    try {
      ...
      await showAlert('일정 목록을 불러오지 못했습니다.');
    } finally {
  ...
  const submitChange = async () => {
    if (!selectedNewDate) {
      await showAlert('변경할 일정을 선택해주세요.');
      return;
    }
    if (!(await showConfirm('정말 해당 일정으로 변경하시겠습니까?\n기존에 입력하신 교통수단, 지하철역 등은 그대로 유지됩니다.'))) return;
    ...
    try {
      ...
      if (result.status === 'success') {
        await showAlert('일정 변경 처리가 완료되었습니다.\n새로운 일정에서 뵙겠습니다!');
        setRecord((prev: any) => {
          let newDday: number | '?' = '?';
        ...
        await showAlert(result.message || '일정 변경 중 오류가 발생했습니다.');
      }
    } catch (err) {
      await showAlert('네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
    } finally {
      setIsChangeProcessing(false);
    }
  };

  const handleCancel = (scheduleText: string, category: string = '유기견 봉사', dday: number | '?' = '?') => {
    ...
    setCancelConfirmState({ scheduleText, category, msg: confirmMsg });
  };

  const executeCancel = async () => {
    if (!cancelConfirmState) return;
    const { scheduleText, category } = cancelConfirmState;
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        await showAlert(result.message || '취소 처리 중 오류가 발생했습니다.');
      }
    } catch (err: any) {
      await showAlert('네트워크 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-[fadeIn_0.2s_ease]">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-red-50/50">
              <h3 className="font-bold text-red-800 text-lg">일정 취소 확인</h3>
              <button onClick={() => setCancelConfirmState(null)} className="text-gray-400 hover:text-gray-600 transition">
                <X size={20} />
              </button>
            </div>
            <div className="p-6">
              <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">
                {cancelConfirmState.msg}
              </p>
            </div>
            <div className="p-5 border-t border-gray-100 bg-gray-50 flex gap-3">
              <button
                onClick={() => setCancelConfirmState(null)}
              ...
              <button
                ...
                className="flex-1 py-3 bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 transition shadow-sm hover:shadow-md"
              >
                네, 취소합니다
              </button>
            </div>
